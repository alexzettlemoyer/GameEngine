...
{
    if (ClientGameState::getInstance() -> playing)
    {    
        switch ( eventType )
        {
            case Event::C_UP:           // character up
                scriptManager -> runOne("handle_up_event", true, "object_context");
                break;
            case Event::C_DOWN:         // character down
                scriptManager -> runOne("handle_down_event", true, "object_context");
                break;
            case Event::C_LEFT:         // character left
                scriptManager -> runOne("handle_left_event", true, "object_context");
                break;
            case Event::C_RIGHT:        // character up
                scriptManager -> runOne("handle_right_event", true, "object_context");
                break;
        }
    }
}
...
void EventHandler::handleCharacterRespawn(std::shared_ptr<Event> e)
{
    ...
    // charRef -> respawn();
        ...
        // std::cout << "New Event: character spawn" << std::endl;
...
void EventHandler::handleCharacterSpawn(std::shared_ptr<Event> e)
{
    ...
    scriptManager -> runOne("handle_game_start_event", false, "object_context");
    scriptManager -> runOne("handle_new_piece_event", false, "object_context");
...
void EventHandler::handlePause(std::shared_ptr<Event> e)
{
    ...
    // timeline -> pause();
    scriptManager -> runOne("handle_pause_event", false, "object_context");
}
